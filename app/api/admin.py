...
def add_book(
    ...
    result = book_service.add_book(req.model_dump())

    if not result.get("success"):
        raise HTTPException(
            status_code=400,
            detail=result.get("message", "Unable to add book.")
        )
...
def update_book_copies(
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=400,
            detail=result.get("message", "Unable to update copies.")
        )
